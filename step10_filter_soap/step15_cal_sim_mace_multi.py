...
mols=read(args.input_file, index=":")
...
for id_i, mol_a in enumerate(mols):
    if id_i>200:  # to limit the calculation for testing
        feat_a = np.load(f"node_feats/node_feats_{id_i}.npy", allow_pickle=True)
        for id_j, mol_b in enumerate(mols):    
           
                # print("Calculating dissimilarity between molecule {} and {}".format(id_i, id_j))

            feat_b = np.load(f"node_feats/node_feats_{id_j}.npy", allow_pickle=True)
