...
save_prefix='all_posible_confs_unreason'
...
for i in opt_list:
    if not os.path.exists(f'{i}/mace_opt_traj.xyz'):
        continue
    traj=read(f'{i}/mace_opt_traj.xyz', index='0::5')
    ...
    for mol in traj:
        ...
        mol_max_force=mol.get_forces().max()
        h_too_far=False
        if min_h_c.max()>2.0:
            h_too_far=True
        # if (c_br_1>2.0 or c_br_2>2.0) and (upper_br_pos_max<16) and (not h_too_far) and (mol_max_force<1000) and (mol_max_force>100):
        if  (not h_too_far) and (mol_max_force<1000) and (mol_max_force>100):   
            mol_num=mol_num+1
            print('Found it!')
            print('C-Br1:', c_br_1)
            print('C-Br2:', c_br_2)
...
for i in opt_list:
    if not os.path.exists(f'{i}/mace_md_traj.xyz'):
        continue
    traj=read(f'{i}/mace_md_traj.xyz', index='0::5')
